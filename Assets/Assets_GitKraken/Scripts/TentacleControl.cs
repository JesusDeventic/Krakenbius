...
public class TentacleControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Array.Sort(tentacles, CompareObNames);  // Ordena los tentáculos por nombre alfabéticamente
        rankingManager = GameObject.Find("rankingManager");
    }

    // Maneja colisión con cada tipo de item Git, aplica efectos específicos en los tentáculos y score
    void OnCollisionEnter2D(Collision2D coll)
    {
        string tentagle_collision = coll.gameObject.tag;

        if (tentagle_collision == "Branch")  // Amarillo: +800pts + activa siguiente tentáculo disponible
        {
        ...
        else if (tentagle_collision == "Commit")  // Cyan: +100pts + crece altura del tentáculo
        {
        ...
        else if (tentagle_collision == "Conflict")  // Rojo: resetea ancho | si ancho es igual a 1 > desactiva tentáculo + checkea GameOver
        {
        ...
        else if (tentagle_collision == "Merge")  // Púrpura: +500pts + crece mucho ancho (si ancho es igual a 1)
        {
        ...
        else if (tentagle_collision == "Pull")  // Naranja: +300pts + crece altura x3
        {
        ...
        else if (tentagle_collision == "Push")  // Verde: +200+(altura*100)pts + resetea altura tentáculo
        {
        ...
        else if (tentagle_collision == "Rebase")  // Blanco: resetea TODOS los tentáculos + activa primeros 4 + bonus
        {
    ...
    // Activa GameOver: finaliza la partida, muestra el UI, reproduce un sonido pitch down y destruye Kraken
    private void GameOver()
    {
        rankingManager.GetComponent<DialogRankingController>().FinalOfMatch();
        Instantiate(gameOver);
        UnityEngine.Object.FindFirstObjectByType<AudioManager>().PitchBase();
        UnityEngine.Object.FindFirstObjectByType<HUDPanelController>().GameOver();
        Destroy(GameObject.Find("Kraken"));
    }

    // Comparador para ordenar el array de tentáculos por nombre alfabéticamente
